...
public class PanelCrearGrupo extends javax.swing.JPanel {
    ...
    public PanelCrearGrupo(Usuario usuario, String direccion) {
        initComponents();
    ...
    private void initComponents() {
        ...
        jButton1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/eliminar-usuario.png"))); // NOI18N
        ...
        add(jButton1, new org.netbeans.lib.awtextra.AbsoluteConstraints(370, 180, -1, -1));
    ...
    private void jButtonGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonGuardarActionPerformed
        if (!jTextFieldGrupo.getText().equals("")) {//comprobar campo
            if (bandera == true) {
                if (listaAmigosAuxiliar.size() > 0) {//verificamos la existencia de elementos en la lista
                    jLabelControl1.setVisible(false);
                    //creamos un objeto grupo
                    Grupo group = new Grupo(jTextFieldGrupo.getText(), jTextAreaDescripcion.getText(), listaAmigosAuxiliar);
                    //creamos un unsario modificado que va almacinar el grupo
                    usuarioActual.getListaGrupos().add(group);
                    Usuario modificado = new Usuario(usuarioActual.getNombre(), usuarioActual.getCorreo(), usuarioActual.getClave(), usuarioActual.getImagen(),
                            usuarioActual.getListaAmigo(), usuarioActual.getListaEnviados(), usuarioActual.getListaRecibidos(), usuarioActual.getListaGrupos(),usuarioActual.getListaBorrador());
                    //modificamos en en alrchivo

                    file.modificar(modificado, direccionActual);
                    System.out.println("se modifico correctamente");
                    this.setVisible(false);

                    new CambiaPanel(Menu.pnlPrincipal, new PanelGrupo(modificado, direccionActual));

                } else {
                    JOptionPane.showMessageDialog(null, "debe agregar un amigo a lista");
